...
- (BOOL)yb_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGPathAddRect(Path, NULL, CGRectMake(0, 0, self.frame.size.width, self.frame.size.height + 20));
    
    CTFrameRef frame = CTFramesetterCreateFrame(framesetter, CFRangeMake(0, 0), Path, NULL);
    
    CFArrayRef lines = CTFrameGetLines(frame);
    
    CGFloat total_height =  [self yb_textSizeWithAttributedString:self.attributedText width:self.frame.size.width numberOfLines:0].height;
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGFloat lineOutSpace = (self.frame.size.height - total_height) / 2;
...
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.frame.size.height), 1.f, -1.f);
}
...
- (CGRect)yb_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    ...
    height = [self yb_textSizeWithAttributedString:attributedString width:self.frame.size.width numberOfLines:0].height;
...
- (CGFloat)yb_getLineOrign:(CTLineRef)line
{
    ...
    if (range.location == 0) {
        ...
        return [self yb_textSizeWithAttributedString:attributedString width:self.frame.size.width numberOfLines:0].height;
    }
}
